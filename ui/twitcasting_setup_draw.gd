...
func _draw() -> void:
	...
	draw_rect(Rect2(field_x, field_y, field_w, field_h), Color(0.5, 0.4, 0.9), false, 1.5)
	...
	var can_connect: bool = setup._user_id_input != ""
	...
	# How it works
	var info_y := py + panel_h - 60
	draw_string(ThemeDB.fallback_font, Vector2(px + 30, info_y), "ユーザーIDを入力するだけで配信に自動接続!", HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color(0.45, 0.45, 0.55))
	draw_string(ThemeDB.fallback_font, Vector2(px + 30, info_y + 18), "視聴者は !spawn スライム 等のコマンドで参加", HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color(0.45, 0.45, 0.55))
